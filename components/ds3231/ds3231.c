...
static esp_err_t ds3231_write_reg(ds3231_handle_t handle, uint8_t reg,
  ...
  esp_err_t err = i2c_master_transmit(handle->dev_handle, buf, len + 1U, 100);
  free(buf);
  return err;
}
...
static esp_err_t ds3231_read_reg(ds3231_handle_t handle, uint8_t reg,
  ...
  return i2c_master_transmit_receive(handle->dev_handle, &reg, 1, data, len,
                                     100);
}
...
esp_err_t ds3231_init(const ds3231_config_t *cfg, ds3231_handle_t *out_handle) {
  ...
  esp_err_t err =
      i2c_master_bus_add_device(cfg->i2c_bus, &dev_cfg, &dev->dev_handle);
  if (err != ESP_OK) {
    free(dev);
    return err;
  }
  ...
  uint8_t control = 0;
  err = ds3231_read_reg(dev, DS3231_REG_CONTROL, &control, 1);
  if (err != ESP_OK) {
